...
/**
 * This is the class that models the module whose duty is to 
 * save the data taken by a satelitte each time it makes a mesure.
 * It also manages the mesure sequence of the satelittes.
 */

public class DataSaver {

    /**
     * the attribute DIR1 corresponds to the directory where all the data files are stored.
     */
    private static final String DIR1 = "ProjetJava/DataFiles/";
    /**
     * the attribute DIR1 corresponds to the directory where the files that contains the sequence
     * of the measures for each satellite.
     */
    ...
    /**
     * Once a satellite is told to make a mesure, we wil save this data on a specific file,
     * on the repository ProjetJava/DataFiles.
    ...
    /**
     * This method creates for a satelitte the file /DATANAMESATNEXTSEQ in the repository
     * DIR, where the sequence is stored.
     * If the file already exists, then it does nothing (in oder to save the sequence that was
     * already here).
    ...
    public void CreateSeq(String nameSat) throws IOException{
        ...
        if (directoryListing != null) {
            // We go through all the files of the directory to see wheter the sequence files already exists or not. 
    ...
    /**
     * This method goes to the files where the sequence of a given satelitte is stored, 
     * and it updates it when a mesure is done by the satellite.
     * To get the sequence we use the getSeq method defined below.
